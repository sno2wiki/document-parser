...
export const getRedirect = (
  ...
  if (!array || array.length === 0) {
    if (!context) {
      ...
      return {
        block: { context, term, implict: false, color: 0 },
        override: { term, context },
      };
    }
  } else {
    if (!context) {
      return {
        block: {
          context: array[array.length - 1],
          term,
          implict: true,
          color: array.length - 1,
        },
      };
    } else if (array[array.length - 1] !== context) {
      return {
        block: { context, term, implict: false, color: array.length },
        override: { term, context },
      };
    } else {
      return {
        block: { context, term, implict: false, color: array.length - 1 },
      };
    }
  }
};
...
export const parseMonospace = buildDecoParser<MonospaceBlock>(
  "\\`.+?\\`",
  "MONOSPACE",
);
export const parseStrong = buildDecoParser<StrongBlock>(
  "\\*\\*.+?\\*\\*",
  "STRONG",
);
export const parseItalic = buildDecoParser<ItalicBlock>(
  "\\_\\_.+?\\_\\_",
  "ITALIC",
);
