...
export const parseText = (
  ...
  return parseRecursively(
    { type: "PURE", text },
    contexts,
  ) as DisplayBlockUnion[];
};

const parseRecursively = (
  block: ParsingBlockUnion,
  ctxMap: ContextsMap,
): ParsingBlockUnion[] => {
  switch (block.type) {
    case "PURE": {
      ...
      if (override) {
        ctxMap.set(override.term, [
          ...(ctxMap.get(override.term) || []),
          override.context,
        ]);
      }
      return blocks.reduce(
        (
          prev: ParsingBlockUnion[],
          block,
        ) => [...prev, ...parseRecursively(block, ctxMap)],
        [],
      );
    }
      ...
      return parseMonospace(block.text)
        .reduce(
          (
            ...
            ...prev,
            ...parseRecursively(block as ParsingBlockUnion, ctxMap),
          ],
          [],
        );
